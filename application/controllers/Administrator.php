...
class Administrator extends CI_Controller {
    ...
    function upgrade(){
		...
		echo $this->session->set_flashdata('message', '<div class="alert alert-success"><center>Dashboard Berhasil diperbaharui, Terima kasih..</center></div>');
		redirect($this->uri->segment(1).'/home');
	}
    ...
    public function tambah_aksi(){
        ...
        $gambar = $this->input->post('project_url');
        ...
        $this->template->load('administrator/template','administrator/mod_produk/tambah_produk',$data);
    ...
    function tambah_produk(){
		...
		if (isset($_POST['submit'])){
            ...
            $this->model_app->insert('produk',$data,);
                redirect($this->uri->segment(1).'/dataproduk');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
    ...
    function tambah_client(){
		cek_session_akses('listberita',$this->session->id_session);
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'template\ti-pindad2\assets\img\clients';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '1000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('gambar');
            ...
            $this->model_app->insert('client',$data,);
                redirect($this->uri->segment(1).'/dataclient');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_produk/tambahclient',$data);
		}
	}
    ...
    public function hapus2($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'client');
        redirect('administrator/dataclient');
    }
    public function hapus3($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'fitur');
        redirect('administrator/datafitur');
    }public function hapus4($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'deskripsi');
        redirect('administrator/datadesk');
    }
    public function hapus5($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'aplikasi');
        redirect('administrator/dataapp');
    }
    public function hapus6($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'event');
        redirect('administrator/dataevent');
    }
    public function hapus7($id){
        $where = array('id' => $id);
        $this->model_produk->hapusdata($where, 'kontak');
        redirect('administrator/datakontak');
    }
    function edit_produk(){
		...
		$id = $this->uri->segment(3);
            if (isset($_POST['submit'])){
                ...
                if ($hasil['file_name']=='')
                {
            $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                        'client'=>$this->db->escape_str($this->input->post('client')),
                        'detail_produk'=>$this->db->escape_str($this->input->post('detail_produk')),
                        'kategori'=>$this->db->escape_str($this->input->post('kategori')),
                        'project_date'=>$this->db->escape_str($this->input->post('project_date')),
                        'project_url'=>$this->db->escape_str($this->input->post('project_url'))
                        );
                    }else{
                        $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                        'client'=>$this->db->escape_str($this->input->post('client')),
            ...
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('produk', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('produk', array('id' => $id))->row_array();
            }
            $data = array('rows' => $proses);
    ...
    function edit_client(){
		...
		$id = $this->uri->segment(3);
            if (isset($_POST['submit'])){
                $config['upload_path'] = 'template/ti-pindad2/assets/img/clients';
                $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
                $config['max_size'] = '3000'; // kb
                $this->load->library('upload', $config);
                $this->upload->do_upload('gambar');
            ...
            if ($hasil['file_name']=='')
            {
                $data = array('detail_client'=>$this->db->escape_str($this->input->post('detail_client')));
            }else{
                $data = array('detail_client'=>$this->db->escape_str($this->input->post('detail_client')),
                        'gambar'=>$hasil['file_name']
                        );
            ...
            $where = array('id' => $this->input->post('id'));
                $this->model_app->update('client', $data, $where);
            
			redirect($this->uri->segment(1).'/dataclient');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('client', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('client', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_produk/view_client_edit',$data);
		}
	}
    function datafitur(){
		cek_session_akses('manajemenuser',$this->session->id_session);
		$data['record'] = $this->model_app->view_ordering('users','username','DESC');
		$this->template->load('administrator/template','administrator/mod_fitur/view_fitur',$data);
	}
    function tambah_fitur(){
		cek_session_akses('listberita',$this->session->id_session);
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'template/ti-pindad2/assets/img/fitur';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '1000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('gambar');
            ...
            $detail_nama = $this->input->post('detail_nama');
				$detail_fitur = $this->input->post('detail_fitur');
            ...
            $this->model_app->insert('fitur',$data,);
                redirect($this->uri->segment(1).'/datafitur');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_fitur/tambahfitur',$data);
		}
	}
    function edit_fitur(){
		cek_session_akses('manajemenmodul',$this->session->id_session);
		$id = $this->uri->segment(3);
            ...
            {
                $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                              'detail_fitur'=>$this->db->escape_str($this->input->post('detail_fitur'))
                             );
            }else{
                $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                            'detail_fitur'=>$this->db->escape_str($this->input->post('detail_fitur')),
            ...
            $where = array('id' => $this->input->post('id'));
                $this->model_app->update('fitur', $data, $where);
            
			redirect($this->uri->segment(1).'/datafitur');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('fitur', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('fitur', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_fitur/view_fitur_edit',$data);
		}
	}
    function datadesk(){
		cek_session_akses('manajemenuser',$this->session->id_session);
		$data['record'] = $this->model_app->view_ordering('users','username','DESC');
		$this->template->load('administrator/template','administrator/mod_deskripsi/view_desk',$data);
	}
    function tambah_desk(){
		cek_session_akses('listberita',$this->session->id_session);
		if (isset($_POST['submit'])){
            $detail_nama = $this->input->post('detail_nama');
                ...
                $bg_desk = $this->input->post('bg_desk');
            ...
            $this->model_app->insert('deskripsi',$data,);
                redirect($this->uri->segment(1).'/datadesk');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_deskripsi/tambahdesk',$data);
		}
	}
    function edit_desk(){
		cek_session_akses('manajemenmodul',$this->session->id_session);
		$id = $this->uri->segment(3);
            ...
            if ($hasil['file_name']=='')
            {
                $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                              ...
                              'warna'=>$this->db->escape_str($this->input->post('warna')),
                              'bg_desk'=>$this->db->escape_str($this->input->post('bg_desk'))
                             );
            }else{
                $data = array('detail_nama'=>$this->db->escape_str($this->input->post('detail_nama')),
                            'deskripsi'=>$this->db->escape_str($this->input->post('deskripsi')),
            ...
            $where = array('id' => $this->input->post('id'));
                $this->model_app->update('deskripsi', $data, $where);
            
			redirect($this->uri->segment(1).'/datadesk');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('deskripsi', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('deskripsi', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_deskripsi/view_desk_edit',$data);
		}
	}
    function dataapp(){
		cek_session_akses('manajemenuser',$this->session->id_session);
		$data['record'] = $this->model_app->view_ordering('users','username','DESC');
		$this->template->load('administrator/template','administrator/mod_app/view_app',$data);
	}
    function tambah_app(){
		cek_session_akses('listberita',$this->session->id_session);
		if (isset($_POST['submit'])){
            ...
            $this->model_app->insert('aplikasi',$data,);
                redirect($this->uri->segment(1).'/dataapp');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_app/tambahapp',$data);
		}
	}
    function edit_app(){
		cek_session_akses('manajemenmodul',$this->session->id_session);
		$id = $this->uri->segment(3);
            ...
            $where = array('id' => $this->input->post('id'));
                $this->model_app->update('aplikasi', $data, $where);
            
			redirect($this->uri->segment(1).'/dataapp');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('aplikasi', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('aplikasi', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_app/view_app_edit',$data);
		}
	}
    function dataevent(){
		cek_session_akses('manajemenuser',$this->session->id_session);
    ...
    function tambah_event(){
		cek_session_akses('listberita',$this->session->id_session);
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'template/ti-pindad2/assets/img/event';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '1000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('gambar');
            ...
            $this->model_app->insert('event',$data,);
                redirect($this->uri->segment(1).'/dataevent');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_event/tambahevent',$data);
		}
	}
    function edit_event(){
		cek_session_akses('manajemenmodul',$this->session->id_session);
		$id = $this->uri->segment(3);
            ...
            if ($hasil['file_name']=='')
            {
                $data = array('nama'=>$this->db->escape_str($this->input->post('nama')),
                              'deskripsi'=>$this->db->escape_str($this->input->post('deskripsi')),
                              'tim'=>$this->db->escape_str($this->input->post('tim'))
                             );
            }else{
                $data = array('nama'=>$this->db->escape_str($this->input->post('nama')),
                            'deskripsi'=>$this->db->escape_str($this->input->post('deskripsi')),
            ...
            if ($this->session->level=='admin'){
                ...
                $proses = $this->model_app->edit('event', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
    ...
    function datakontak(){
		cek_session_akses('manajemenuser',$this->session->id_session);
		$data['record'] = $this->model_app->view_ordering('users','username','DESC');
		$this->template->load('administrator/template','administrator/mod_kontak/view_kontak',$data);
	}
    function tambah_kontak(){
		...
		if (isset($_POST['submit'])){
            ...
            $this->model_app->insert('kontak',$data,);
                redirect($this->uri->segment(1).'/datakontak');
		}else{
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_kontak/tambahkontak',$data);
		}
	}
    function edit_kontak(){
		...
		$id = $this->uri->segment(3);
            ...
            {
                $data = array('nama'=>$this->db->escape_str($this->input->post('nama')),
                              'akun'=>$this->db->escape_str($this->input->post('akun')),
            ...
            $where = array('id' => $this->input->post('id'));
                $this->model_app->update('kontak', $data, $where);
            
			redirect($this->uri->segment(1).'/datakontak');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('kontak', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('kontak', array('id' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_kontak/view_kontak_edit',$data);
		}
	}
}
